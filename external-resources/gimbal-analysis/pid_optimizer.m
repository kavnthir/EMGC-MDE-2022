...
% analyzes response of multiple systems with varying PID parameters
% and determines which set of parameters result in the best response.
...
bestParameters.Px_roll = FP.Px_roll;
bestParameters.Ix_roll = FP.Ix_roll;
bestParameters.Dx_roll = FP.Dx_roll;
bestParameters.SSE_roll = Results.SSE_roll;
...
for Dx = 0 : 1 : 0
    for Ix = 0 : 0.1 : 1
        for Px = 0 : 0.2 : 2.4
            ...
            Results = AnalizeResponse(FP, Response);
            PlotResponse(FP, Response, Results, 'Response');
            disp("Px: "+Px+", Ix: "+Ix+", Dx: "+Dx+", SSE: "+Results.SSE_roll);
            
            if (Best.Results.SSE_roll > Results.SSE_roll)
                Best.FP = FP;
                Best.Response = Response;
                Best.Results = Results;
            end
...
PlotResponse(Best.FP, Best.Response, Best.Results, 'Best Response');
